...
class PrescriptionRepoImpl : PrescriptionRepo {
    ...
    override fun addPrescription(model: PrescriptionModel, callback: (Boolean, String) -> Unit) {
        val id = ref.push().key ?: ""
        val newModel = model.copy(prescriptionId = id)
        ref.child(id).setValue(newModel)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    callback(true, "Prescription submitted successfully")
                } else {
                    callback(false, task.exception?.message ?: "Failed to submit prescription")
                }
            }
    }
